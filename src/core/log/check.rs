...
impl Display for CheckLogs {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            CheckLogs::Welcome => f.write_str("🥳 Welcome to use gpiler checker!"),
            ...
            CheckLogs::DependenceNotFound(name) => f.write_fmt(format_args!(
                "❗️ {} is not found, you can run `gpiler install` to install in default chain path or run `gpiler config` to set the path",
                name
            )),
